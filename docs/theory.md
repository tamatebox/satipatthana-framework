...
**Grounding Metrics** は収束前の情報を捉えることでこれを解決する：

1. **s0_min_dist** — 初期状態 $S_0$ から最近接プローブへの距離。高い値は、入力が収束 *前に* 既知の概念から遠かったことを示す。
2. **drift_magnitude** — Vicaraが状態をどれだけ動かしたか（$\|S^* - S_0\|$）。大きなドリフトは、入力が偽のアトラクタに「引き込まれた」ことを示唆する。
3. **recon_error** — 現実確認としての再構成誤差。高い誤差は幻覚を示す。

これらのメトリクスにより、「自信満々に間違っている」状態 — 滑らかに収束するが誤った領域へ向かう入力 — の検出が可能になる。
