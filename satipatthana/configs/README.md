# Configuration System for Satipatthana Framework

This directory contains the new Type-Safe Dataclass-based configuration system for the Satipatthana Framework, replacing the old `Dict[str, Any]` based configuration.
...
- **Single Source of Truth**: `SystemConfig` acts as the root configuration, encapsulating all component-specific configurations.
...
- `main.py`: Contains `SystemConfig`, the top-level configuration dataclass that holds instances of all other component configurations.

## Usage Example

### 1. Creating a `SystemConfig`

YouYou can create a `SystemConfig` directly or from a dictionary:

```python
from satipatthana.configs.main import SystemConfig
from satipatthana.configs.enums import AdapterType, VicaraType, DecoderType
from satipatthana.configs.objectives import ObjectiveConfig
...
my_config = SystemConfig.from_dict(config_data)
...
```python
# In SatipatthanaSystem or other components:
class MyComponent:
...
5. If it's a top-level parameter or a new nested component (like `objective`), update `SystemConfig` in `main.py`.
