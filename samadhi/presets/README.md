...
1.  **Create a New File**: Add a new Python file (e.g., `samadhi/presets/my_new_preset.py`) in this directory.
...
4.  **Use `SamadhiBuilder`**: Use `SamadhiBuilder` to assemble these components into an `SamadhiEngine` instance.
    ...
    from samadhi.components.adapters.sequence import LstmAdapter
    from samadhi.components.decoders.sequence import LstmDecoder
